...
# Import Measurements & Forecasts Models
...
# Import Core Models (New additions)
...
# Updated Schema: Added fields to allow returning real data
...
# Updated Schema: Added fields to match Device model
...
# Updated Schema: Added fields to match UserAccount model
class UserSchema(BaseModel):
    id: str
    username: str  # Will map to 'email' from DB
    password_hash: str
    roles: List[str]
...
class DataAccessGateway(IMeasurement, IForecastRead, IForecastWrite, ICoreDb):
    ...
    # =========================================================
    # 3. CORE DB (Implemented per Friend's Instructions)
    # =========================================================
    
    async def get_building(self, building_id: str) -> Optional[BuildingMetadata]:
        try:
            # Query the 'building' collection
    ...
    async def get_devices(self, building_id: str, device_type: Optional[str] = None, status: Optional[str] = "active") -> List[DeviceMetadata]:
        # Filter devices by building_id (possible because you added the field to core.py!)
    ...
    async def get_device(self, device_id: str) -> Optional[DeviceMetadata]:
        # Find specific device by its hardware ID
    ...
    async def get_user_by_username(self, username: str) -> Optional[UserSchema]:
        # Logic: Alerts team asks for 'username', but Database uses 'email'.
        # We look up the 'email' column using the provided username.
        u = await UserAccount.find_one(UserAccount.email == username)
        if not u:
            return None
            
        return UserSchema(
            id=str(u.id),
            username=u.email, # Return email as the username
